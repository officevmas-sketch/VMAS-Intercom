...
function renderAdminEmployees(){
  ...
  $('adminEmployeeList').innerHTML = rows.map(([username,e]) => {
    const isSelf = username === currentUser;
    const active = e.active !== false;
    return `<div class="admin-employee ${active ? '' : 'inactive'}">
      <div>
      ...
      <div class="admin-actions">
        <button class="secondary small-btn" onclick="prefillEmployee('${escapeJs(username)}')">Edit</button>
        <button class="secondary small-btn" ${isSelf?'disabled':''} onclick="toggleEmployeeActive('${escapeJs(username)}')">${active ? 'Deactivate' : 'Activate'}</button>
        <button class="danger small-btn" ${isSelf?'disabled':''} onclick="deleteEmployee('${escapeJs(username)}')">Delete</button>
      </div>
    </div>`;
  }).join('') || '<p class="muted">No employees found.</p>';
}

window.prefillEmployee = function(username){
  const e = employees[username]; if (!e) return;
...
window.toggleEmployeeActive = async function(username){
  ...
  await db.ref(`employees/${username}`).update({active: next});
  await db.ref(`presence/${username}`).remove();
  if (!next) await db.ref(`calls/${username}`).remove();
  toast(next ? 'Employee activated' : 'Employee deactivated');
}
...
window.deleteEmployee = async function(username){
  ...
  const e = employees[username]; if (!e) return;
  if (!confirm(`Delete ${e.name || username}? This cannot be undone.`)) return;
  await db.ref(`employees/${username}`).remove();
  await db.ref(`presence/${username}`).remove();
  await db.ref(`calls/${username}`).remove();
